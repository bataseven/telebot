...
void SerialHandler::parseString(char *string) {
    ...
    case 'V':
        ...
        robot->arm->desiredCartesianVelocity(0) = atof(token) * 5;
        float val = map(atof(token), -1, 1, 35, 160);
        robot->arm->val = val;

        token = strtok(NULL, seperator);
        robot->base->setDesiredVy(atof(token));
        robot->arm->desiredCartesianVelocity(1) = atof(token) * 5;

        token = strtok(NULL, seperator);
        robot->base->setDesiredWz(atof(token));
        robot->arm->desiredCartesianVelocity(2) = atof(token) * 5;
