...
char WORKING_MODE = 'M'; // M for manual, R for recoding, P for repeating the recorded pattern, C for otomatik, S for stop actuating motors
...
void setup() {
  ...
  ROBOT_STEPS[0] =  stepPerRev * beltRatio2 * MICRO_STEPPING_ARM * RECEIVED_ANGLES[0] / 360 + ROBOT_STEPPER_MAX_POSITION_0 / 2;
  ROBOT_STEPS[1] = ROBOT_STEPPER_MAX_POSITION_1;
  ROBOT_STEPS[2] =  stepPerRev * beltRatio  * gearRatio * MICRO_STEPPING_ARM * RECEIVED_ANGLES[1] / 360 + 5000; // Convert received angle to steps
...
void loop() {
  ...
  if (newData || WORKING_MODE == 'S') { // Only enter if newData is available. That way robot follows the commands in discrete time steps.

    switch (WORKING_MODE) {
      case 'R': //  RECORD
        ROBOT_STEPS[2] =  stepPerRev * beltRatio * gearRatio * MICRO_STEPPING_ARM * RECEIVED_ANGLES[1] / 360 + 5000;
        ROBOT_STEPS[0] = -stepPerRev * beltRatio2 * MICRO_STEPPING_ARM * RECEIVED_ANGLES[0] / 360 + ROBOT_STEPPER_MAX_POSITION_0 / 2;
        ...
        for (int i = 0; i < 3; i++) ROBOT_STEPS_RECORDED[i][RECORD_INDEX]  = ROBOT_STEPPER_MOTORS[i].currentPosition();
        for (int i = 0; i < 4; i++) WHEEL_STEPS_RECORDED[i][RECORD_INDEX]  = WHEEL_STEPPER_MOTORS[i].currentPosition();
        for (int i = 0; i < 4; i++) WHEEL_SPEEDS_RECORDED[i][RECORD_INDEX] = WHEEL_STEPPER_MOTORS[i].speed();
        RECORD_INDEX++;
        RECORD_COUNT++;
        if (RECORD_INDEX == RECORD_COUNT_MAX) WORKING_MODE = 'P'; // Automatically starts playing when the record array is full
        break;
      ...
      case 'P': //  PLAY
        if (RECORD_COUNT != 0) {
          if (WORKING_MODE_PREVIOUS != 'P') {
            PLAY_INDEX = 0;
            for (int i = 0; i < 4; i++) WHEEL_STEPPER_MOTORS[i].setCurrentPosition(WHEEL_STEPS_RECORDED[i][PLAY_INDEX]);
          }
          if (PLAY_INDEX == 0)  for (int i = 0; i < 4; i++) WHEEL_STEPPER_MOTORS[i].setCurrentPosition(WHEEL_STEPS_RECORDED[i][PLAY_INDEX]);
          for (int i = 0; i < 4; i++) {
      ...
      case 'S': // STOP
        for (int i = 0; i < 4; i++)
          WHEEL_STEPPER_MOTORS[i].setSpeed(0);
        break;
    }
  }
...
void moveSteppers() {
  ...
  if (WORKING_MODE != 'P')  for (int i = 0; i < 4; i++)  WHEEL_STEPPER_MOTORS[i].runSpeed();
  else {
    for (int i = 0; i < 4; i++) {
...
void setMotorSpeed(String Data) {
  ...
  if (yVel != 0 || xVel != 0) {
    ...
    if (Velocity_Vector_Angle < 0)
      Velocity_Vector_Angle += 360;
    ...
    if (Velocity_Vector_Angle >= 0 && Velocity_Vector_Angle <= 90) {
      ...
      WHEEL_SPEEDS[1] = -map(Velocity_Vector_Angle, 0, 90, -WHEEL_STEPPER_MAX_SPEED * X, WHEEL_STEPPER_MAX_SPEED * X) ;
      WHEEL_SPEEDS[3] = -WHEEL_SPEEDS[1];
    }
    else if (Velocity_Vector_Angle > 90 && Velocity_Vector_Angle <= 180) {
      WHEEL_SPEEDS[0] = map(Velocity_Vector_Angle, 90, 180, WHEEL_STEPPER_MAX_SPEED * X, -WHEEL_STEPPER_MAX_SPEED * X);
      WHEEL_SPEEDS[2] = -WHEEL_SPEEDS[0];
      ...
      WHEEL_SPEEDS[0] = -WHEEL_STEPPER_MAX_SPEED * X ;
      WHEEL_SPEEDS[2] = -WHEEL_SPEEDS[0];
      WHEEL_SPEEDS[1] = -map(Velocity_Vector_Angle, 180, 270, WHEEL_STEPPER_MAX_SPEED * X, -WHEEL_STEPPER_MAX_SPEED * X) ;
      WHEEL_SPEEDS[3] = -WHEEL_SPEEDS[1];
    }
    else {
      WHEEL_SPEEDS[0] = map(Velocity_Vector_Angle,  270, 360, -WHEEL_STEPPER_MAX_SPEED * X, WHEEL_STEPPER_MAX_SPEED * X) ;
      WHEEL_SPEEDS[2] = -WHEEL_SPEEDS[0];
      WHEEL_SPEEDS[1] = WHEEL_STEPPER_MAX_SPEED * X;
      WHEEL_SPEEDS[3] = -WHEEL_SPEEDS[1];
    }
