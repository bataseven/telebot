...
class Melodies  {
  public:
    // Constructor
    Melodies(int buzzerpin = 20);

    // Methods
    void play(int selectedSong);
    void changePin(int buzzerpin);
    int getCurrentPin();

  private:
    int _BuzzerPin;
    Metro _noteTimer = Metro(1);
    int _Selected_Song = 0;
    int _Selected_Song_Prev = 0;
    int _noteToPlay = 0;
    int _tempo = 1;
    int _notes = 1;
    int _thisNote = 0;
    int _noteDuration = 0;
    int _wholenote = (60000 * 4) / _tempo;  // Duration of a whole note
    int _divider = 1;
    boolean _Song_Changed = false;

    boolean _started = false; /// I have no idea i need these 3 variables
    int _start_Threshold = 10;/// but without them, if the integer passed into
    int _start_Counter = 0;// play fucntion is anything other than '0', song doesnt play for a while


    ///Tempos/////
    int HarryPotter_tempo = 144;
    int LionSleeps_tempo = 122;
    int Astronomia_tempo = 126;
    int LowBattery_tempo = 75;

    int Song_Tempos[Number_Of_Songs + 1] = {1, HarryPotter_tempo, LionSleeps_tempo, Astronomia_tempo, LowBattery_tempo};

    ///Melodies for the songs///
    int LowBattery[4] = {1000, 16 , 0 , 16};
    int Astronomia[212] = {
      REST , 8, REST , 8,
